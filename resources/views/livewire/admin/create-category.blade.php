    ...
    <x-jet-form-section submit="save" class="mb-6">
        ...
        <x-slot name="form">
                    ...
                    @foreach ($brands as $brand)
                        <x-jet-label>
                            <x-jet-checkbox 
                                ...
                                value="{{ $brand->id }}"
                            />
                           <span class="mx-2"> {{ ucwords($brand->name) }}</span>
                        </x-jet-label>
                    @endforeach
                </div>
            ...
            <div class="col-span-6 sm:col-span-4">
                ...
                <input type="file" wire:model="createForm.image" class="mt-1" name="" id="{{$rand}}" accept="image/*">
        ...
        <x-slot name="content">
            ...
            <div class="space-y-3">
                ...
                <div>
                    ...
                    {{-- <p>{{ implode(', ', $editForm['brands']) }}</p>
                    <p>{{ count($editForm['brands']) . ' marcas' }}</p> --}}
                    <div class="grid grid-cols-4">
                        @foreach ($brands as $brand)
                            <x-jet-label>
                                <x-jet-checkbox 
                                    wire:model="editForm.brands"
                                    name="brands[]"
                                    value="{{ $brand->id }}"
                                />
                               <span class="mx-2"> {{ ucwords($brand->name) }} [{{$brand->id}}]</span>
                            </x-jet-label>
                        @endforeach
                    </div>
                ...
                <div>
                    ...
                    <input type="file" wire:model="editImage" class="mt-1" name="" id="{{$rand}}" accept="image/*">
